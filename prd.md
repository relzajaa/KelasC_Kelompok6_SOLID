...
### 1.2 Ruang Lingkup
Sistem akan memodelkan berbagai entitas hewan dengan perilaku spesifiknya (seperti berjalan, terbang, atau berenang), pengelolaan penyimpanan hewan dalam kandang, perawatan hewan, serta rutinitas pembersihan. Arsitektur kode dirancang agar sangat modular, dapat diperluas, dan mudah dirawat.

---

## 2. Deskripsi Sistem
Program "Simulasi Kebun Binatang" adalah aplikasi Command Line Interface (CLI) berbasis Python yang mampu:
...
- Merawat seluruh hewan dengan memastikan setiap entitas melakukan aktivitasnya (makan, berjalan, berenang, terbang) sesuai dengan kapasitas dan antarmuka (interface) yang dimilikinya secara aman.
...
- **Perilaku Dasar:** Setiap hewan harus memiliki kemampuan dasar yaitu `makan()`, `tidur()`, dan `suara()` yang spesifik.
- **Perilaku Khusus (Interface Segregation):** Kemampuan khusus seperti berjalan, berenang, dan terbang harus diimplementasikan secara terpisah melalui Interface/Class Abstrak yang spesifik (misal: `HewanBerjalan`, `HewanBerenang`, `HewanTerbang`).
...
- **Fungsi Menampilkan:** Sistem harus memiliki modul terpisah (`PenampilKandang`) untuk menampilkan daftar seluruh hewan yang ada di dalam Kandang beserta urutannya.
- **Penanganan Kondisi Kosong:** Jika kandang kosong, sistem harus memberikan notifikasi yang sesuai.

### 3.4 Pembersihan Kandang
- **Simulasi Pembersihan:** Sistem memiliki fungsi spesifik (`PembersihKandang`) untuk mensimulasikan proses pembersihan kandang.
...
1. **S - Single Responsibility Principle (SRP):** Tanggung jawab dipecah ke class `Kandang` (penyimpanan), `PenampilKandang` (tampilan), dan `PembersihKandang` (utilitas kebersihan).
2. **O - Open/Closed Principle (OCP):** Kode terbuka untuk diperluas. Penambahan hewan baru (contoh: Buaya) hanya perlu mewarisi class dasar dan interface yang relevan tanpa harus memodifikasi source code class `Hewan` atau class hewan lainnya.
...
5. **D - Dependency Inversion Principle (DIP):** `KebunBinatang` menerima `Kandang` melalui injeksi dependensi, sehingga modul tingkat tinggi tidak bergantung kuat pada modul tingkat rendah secara kaku.
